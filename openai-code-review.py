...
try:
    ...
    print(f"""OpenAI API request took too long to complete:
        {e}""")
    feedback_message = f"""Open AI API request took too long to complete:
        {e}"""

except openai.AuthenticationError as e:
    print(f"""OpenAI API request failed to authenticate the API Key:
        {e}""")
    feedback_message = f"""Open AI API request failed to authenticate the API Key:
        {e}"""

except openai.BadRequestError as e:
    print(f"""OpenAI API request was malformed or is missing parameters:
        {e}""")
    feedback_message = f"""Open AI API request was malformed or is missing parameters:
        {e}"""

except openai.InternalServerError as e:
    print(f"""OpenAI API request reached an Internal Server Error:
        {e}""")
    feedback_message = f"""Open AI API request reached an Internal Server Error:
        {e}"""

except openai.RateLimitError as e:
    print(f"""OpenAI API request has exceeded the Rate Limit:
        {e}""")
    feedback_message = f"""Open AI API request has exceeded the Rate Limit:
        {e}"""

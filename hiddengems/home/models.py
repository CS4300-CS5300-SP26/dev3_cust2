...
class Game(models.Model):
    ...
    authorized_users = models.ManyToManyField(
        User, related_name="authorized_games", blank=True
    )
...
class Rating(models.Model):
    ...
    class Meta:
        unique_together = ("user", "game")  # each user can rate a game once

    def __str__(self):
        return f"{self.user.username} rated {self.game.title} {self.score}/5"
...
class SimilarGame(models.Model):
    game = models.ForeignKey(
        Game, on_delete=models.CASCADE, related_name="similar_games"
    )
    similar = models.ForeignKey(
        Game, on_delete=models.CASCADE, related_name="similar_to"
    )

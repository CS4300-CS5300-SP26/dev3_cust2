...
class GameDetailTests(TestCase):
    ...
    def test_game_detail_page_loads(self):
        ...
        response = self.client.get(
            reverse("game_detail", args=[self.game.slug])
        )
    ...
    def test_duplicate_title_gets_unique_slug(self):
        # Two games with the same title should get different slugs
    ...
    def test_game_detail_404_on_bad_slug(self):
        ...
        response = self.client.get(
            reverse("game_detail", args=["nonexistent-slug"])
        )
        self.assertEqual(response.status_code, 404)


class PurchaseGameTests(TestCase):

    def setUp(self):
        self.client = Client()
        self.game = Game.objects.create(
            title="Purchasable Game",
            description="A game you can buy",
            price="29.99",
            storefront="steam",
            game_id=99999,
        )
    ...
    def test_purchase_page_loads(self):
        ...
        response = self.client.get(
            reverse("purchase_game", args=[self.game.game_id])
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_purchase_page_404_on_bad_game_id(self):
        # Invalid game_id should return 404
        response = self.client.get(reverse("purchase_game", args=[00000]))
        self.assertEqual(response.status_code, 404)
